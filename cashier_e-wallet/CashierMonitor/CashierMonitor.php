...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['done'])) {
        ...
        SET queueStatus = 'Serving', status = 'Serving' 
        WHERE orderID = '$orderId'";

        mysqli_query($conn, $updateProductStatusSql);

        $order_id = $_POST['order_id'];
        mysqli_query($conn, "UPDATE `orders` SET queueStatus = 'Serving' WHERE order_id = '$order_id'");
    }
...
$sql = "SELECT * FROM `ordersdetails` WHERE  queueStatus = 'Completed' AND ($categoryCondition)";
        ...
        <div class="menu-container">
            <main id="orders-container">
                ...
                foreach ($orders as $row) {
                    ...
                    echo '<br>';
                    echo '<li style="color:black;" >' . $row['quantity'] . ' ' . $row['name'] . '</li>';
                    foreach (explode(', ', $row['ingredient']) as $ingre) {
                        ...
                        if (count($parts) === 2) {
                            ...
                            if ((int) $quantity > 1) {
                                echo '<li style="text-align: left; margin-left: 10px; color: black;">↳ ' . $quantity . ' ' . $ingredient . '</li>';
                            }
                        }
                    }
